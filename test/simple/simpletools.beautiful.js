...
!function(window) {
    var default_config = {
        ...
        compress: !0,
        strict: !0
    }, KEYWORD_PREG = "^\\s*((?:/)?(?:" + "if,else,each,include,while,for".split(",").join("|") + "))(.*)", ENGINE = "".trim ? [ "$_tpl_=''", "$_tpl_+=", ";", "$_tpl_" ] : [ "$_tpl_=[]", "$_tpl_.push(", ");", "$_tpl_.join('')" ], escape = {
        "<": "&#60;",
        ">": "&#62;",
        '"': "&#34;",
        "'": "&#39;",
    ...
    var _echo = function(value) {
        ...
        if (obj = value, "[object Array]" === Object.prototype.toString.call(obj)) _arrayEach(value, callback); else for (var index in value) callback.call(value[index], value[index], index);
        var obj;
    ...
    function reportError(name, content, line, e) {
        ...
        if (console.group(report), content) {
            var codes = content.replace(/^\n/, "").split("\n"), start = 0 < line - 5 ? line - 5 : 1, end = line + 5 > codes.length ? codes.length : line + 5;
            console.error(e);
    ...
    function compileTemplate(text, config) {
        ...
        return text = text.replace(/^\n/, ""), _each(text.split(config.tagstart), function(value) {
            ...
            1 === split.length ? tpl += parserHTML(split[0], config.compress) : (tpl += function(code) {
                ...
                switch (command) {
                  case "include":
                    return 1 === (param = param.trim().split(" ")).length && param.push("$_unit.value"), 
                  ...
                  case "each":
                    var match;
                    return (match = param.match(/\s*(.+?)\s+(?:(?:as(?:\s+(\w+)))?(?:\s*:\s*(\w+))?)?/)) ? "$_unit._each(" + match[1] + ",function(" + (match[2] ? match[3] ? match[3] + "," + match[2] : match[2] : "value,index") + "){" : 'throw SyntaxError("Null Each Value");$_unit._each(null,function(){';

                  case "/each":
                    return "});";
                }
            }(split[0]), tpl += parserHTML(split[1]));
        }), tpl;
    }
    function render(name, source, compiled_code, value, strict) {
        var html, runcode = function(source, value, strict) {
            var use_strict = void 0 === strict || strict, ext = [];
            for (var index in ext.push("var $_unit=this," + ENGINE[0]), value) ext.push(index + "=this.value." + index);
            var link_str = "";
            return use_strict && (link_str = '"use strict";'), link_str += ext.join(","), link_str += ";", 
            link_str += source + "return new String(" + ENGINE[3] + ");";
        }(compiled_code, value, strict), caller = {
            _each: _each,
            _echo: _echo,
            _escape: _escape,
            _include: _include,
        ...
        try {
            html = new Function(runcode).call(caller);
        } catch (e) {
            if (match = new String(e.stack).match(/<anonymous>:(\d+):\d+/)) {
                reportError(name, source, match[1] - 1, e);
            } else {
                var match;
                name = name || "anonymous";
                (match = new String(e.stack).match(/Function code:(\d+):\d+/)) ? console.error("DxTPL:Compile Error@" + name + " Line " + match[1]) : console.error("DxTPL:Compile Error@" + name);
            }
        }
        return html;
    }
    function compile(id, config) {
        ...
        anonymous = !0), config.code || (config.cache && !anonymous ? config.code = function(name, config) {
            var cache_parent = document.getElementById("template_caches");
            cache_parent || ((cache_parent = document.createElement("div")).id = "template_caches", 
            ...
            return tpl_cache || ((tpl_cache = document.createElement("div")).id = cache_name, 
        ...
        }(tplId, config) : config.code = compileTemplate(config.source, config)), config;
    }
    var Template = function(name, config) {
        this.version = "1.0.43";
        var conf = default_config;
        "string" == typeof name ? (conf = _objectCopy(conf, config)).id = name : conf = _objectCopy(conf, name), 
        this.config(conf);
    };
    Template.prototype.config = function(config) {
    ...
    }, window.dxtpl = new Template(), window.Template = Template, window.renderTpl = function(selector, glovalue) {
        ...
        _arrayEach(nodes, function(node, index) {
            ...
            if (node.dataset.init) try {
                value = new Function("return " + node.dataset.init + ";")();
            } catch (e) {
                reportError(selector + "[" + index + "]", null, 0, new Error("Unsupport json"));
            }
            if (node.dataset.config) try {
                var conf = new Function("return " + node.dataset.config + ";")();
